...
async def referral_menu(callback: CallbackQuery):
    ...
    friends_count = user[7] or 0
    
    text = (
        f'👥 <b>Реферальная программа</b>\n\n'
        f'💰 Приведи друга и получи <b>0.5$</b> на баланс!\n'
        f'👤 Твоя реферальная ссылка:\n'
        f'<code>{ref_link}</code>\n\n'
        f'📊 Приведено друзей: <b>{friends_count}</b>\n'
        f'💵 Заработано: <b>{friends_count * 0.5:.2f} $</b>\n\n'
        f'📌 <b>Как это работает:</b>\n'
        f'1. Отправь ссылку другу\n'
        f'2. Друг регистрируется в боте\n'
        f'3. Ты получаешь 0.5$ на баланс\n\n'
        f'🎯 <b>Условия:</b>\n'
        f'• Бонус начисляется только за новых игроков\n'
        f'• Без ограничений по количеству друзей'
    )
...
async def bonus_menu(callback: CallbackQuery):
    ...
    welcome_status = '✅ Получен' if has_bonus else '❌ Не получен (пополните баланс)'
    ...
    text = (
        ...
        f'   └ {msg}\n\n'
        f'👥 <b>Реферальная программа</b> доступна в отдельном меню.'
    )
...
async def cmd_start(message: Message):
    ...
    # БОНУС БОЛЬШЕ НЕ ВЫДАЁТСЯ ЗДЕСЬ!
    ...
    text = (
        ...
        f'💰 Баланс: <b>{user[3]:.2f} $</b>\n\n'
        f'🎯 Пополни баланс от 1$ и получи бонус 0.5$!'
    )
...
async def cmd_withdraw(message: Message, state: FSMContext):
    ...
    if user[3] < MIN_WITHDRAW:
        await message.answer(f'❌ Минимальный вывод {MIN_WITHDRAW}$')
        return
...
async def cmd_help(message: Message):
    text = (
        ...
        f'🔹 <code>/withdraw</code> — Вывод средств (мин. {MIN_WITHDRAW}$, нужно пополнить 1$)\n\n'
        '🎮 <b>Игры:</b>\n'
        '• 🎲 Куб (чёт/нечет) — x2\n'
        ...
        '🎁 <b>Бонусы:</b>\n'
        '• Приветственный: 0.5$ (после первого пополнения)\n'
        '• Ежедневный: 0.1$ (пополни 1$ + сыграй 1 игру)\n'
        '• За пополнение: +10%\n'
        '• Кешбэк: 5% от проигрыша\n'
        '• Реферальный: 0.5$ за друга (в меню "Рефералы")\n\n'
        '👑 <b>Админы:</b>\n'
...
async def deposit_menu(callback: CallbackQuery):
    text = (
        ...
        '🎉 <b>Приветственный бонус 0.5$</b> за первое пополнение!\n\n'
        '📝 <b>Инструкция:</b>\n'
...
async def check_payment_callback(callback: CallbackQuery):
    ...
    if status_data['status'] == 'paid':
        ...
        # Выдача приветственного бонуса (если ещё не получен)
        if not await has_welcome_bonus(callback.from_user.id):
            await update_balance(callback.from_user.id, 0.5)
            await mark_welcome_bonus_claimed(callback.from_user.id)
            await callback.message.answer('🎁 Вы получили приветственный бонус 0.5$ за первое пополнение!')
...
async def withdraw_start(callback: CallbackQuery, state: FSMContext):
    user = await get_user(callback.from_user.id)
    if user[3] < MIN_WITHDRAW:
        await callback.answer(f'Минимальный вывод {MIN_WITHDRAW}$', show_alert=True)
        return
    if user[5] < 1.0:
        ...
        try:
            ...
            for invoice_record in pending:
                ...
                if status_data['status'] == 'paid':
                    ...
                    # Выдача приветственного бонуса (если ещё не получен)
                    if not await has_welcome_bonus(user_id):
                        await update_balance(user_id, 0.5)
                        await mark_welcome_bonus_claimed(user_id)
                        try:
                            await bot.send_message(
                                user_id,
                                '🎁 Вы получили приветственный бонус 0.5$ за первое пополнение!'
                            )
                        except:
                            pass
...
async def dice_choice(callback: CallbackQuery):
    ...
    # Показываем анимацию кубика
    msg = await callback.message.answer_dice(emoji=DiceEmoji.DICE)
    await asyncio.sleep(3.5)  # ждем окончания анимации
    
    # Определяем победу по шансу 34%
    roll = random.random()
    is_win = roll < 0.34
    
    # Подбираем значение кубика, чтобы оно соответствовало результату
    # Если победа — показываем чётное (2,4,6), если проигрыш — нечётное (1,3,5)
    # Но также можно показывать любое, но тогда будет несоответствие с результатом,
    # но мы всё равно показываем результат на основе шанса.
    # Чтобы было правдоподобно, генерируем значение кубика, которое соответствует чёт/нечет.
    if is_win:
        # Выигрыш: показываем чётное число
        value = random.choice([2, 4, 6])
        result_text = f'✅ Выигрыш: <b>{amount * 2:.2f} $</b>'
        await update_balance(callback.from_user.id, amount * 2)
    else:
        # Проигрыш: показываем нечётное число
    ...
    # Но мы не можем изменить значение уже отправленного кубика, поэтому мы просто показываем результат
    # с указанием выпавшего числа (которое мы сгенерировали, но оно не соответствует анимации).
    # Чтобы не было расхождения, можно вообще не показывать число, а только результат.
    # Однако пользователь ожидает увидеть число. Лучше показывать сгенерированное число, но это может сбивать с толку.
    # Альтернатива: не показывать число, только результат.
    # Сделаем так: показываем результат с эмодзи кубика, но без указания числа.
    # Или показываем число, но тогда оно может не совпадать с анимацией (что плохо).
    # Лучше показывать реальное значение кубика из анимации, но тогда шанс будет 50%.
    # Я предлагаю не показывать число, а только результат ставки, чтобы не было расхождений.
    # Но пользователь просил анимацию, а не число. Так что мы показываем анимацию, а затем результат без числа.
    # Можно показать результат с сообщением: "Кубик остановился... Результат: ..."
    # Но у нас уже есть анимация, и мы можем просто вывести результат.
    # Я уберу отображение числа, оставлю только результат.
    # Переделаем вывод текста.
...
async def roulette_number_choice(callback: CallbackQuery):
    ...
    if is_win:
        ...
        # Показываем выпавшее число — выбранное игроком (чтобы совпало)
        result_num = number
    else:
        result_text = '❌ Проигрыш'
        cashback = amount * 0.05
        await update_balance(callback.from_user.id, cashback)
        await add_transaction(callback.from_user.id, 'cashback', cashback, 'cashback', f'Кешбэк 5% от {amount:.2f}$')
        # При проигрыше показываем любое число, кроме выбранного
        possible = [i for i in range(37) if i != number]
        result_num = random.choice(possible)
    
    # Анимация с реальным результатом
...
async def set_commands():
    commands = [
        ...
        BotCommand(command='withdraw', description=f'💸 Вывод средств (мин. {MIN_WITHDRAW}$, пополни 1$)'),
