...
async def cmd_withdraw(message: Message, state: FSMContext):
    user = await get_user(message.from_user.id)
    if user[3] < 2.0:
        await message.answer(f'❌ Минимальный вывод 2$')
        return
...
async def cmd_help(message: Message):
    text = (
        ...
        '🔹 <code>/withdraw</code> — Вывод средств (мин. 2$)\n\n'
        '🎮 <b>Игры:</b>\n'
        '• 🎲 Куб (чёт/нечет) — x2\n'
...
async def check_payment_callback(callback: CallbackQuery):
    ...
    if status_data['status'] == 'paid':
        ...
        await callback.answer(f'❌ Статус: {status_data['status']}', show_alert=True)

@dp.callback_query(F.data == 'withdraw')
async def withdraw_start(callback: CallbackQuery, state: FSMContext):
    user = await get_user(callback.from_user.id)
    if user[3] < 2.0:
        await callback.answer(f'Минимальный вывод 2$', show_alert=True)
        return
    await state.set_state(BetState.waiting_withdraw)
    await callback.message.edit_text(
        f'⬆️ <b>Вывод средств</b>\n\n'
...
async def set_commands():
    commands = [
        ...
        BotCommand(command='withdraw', description='💸 Вывод средств (мин. 2$)'),
